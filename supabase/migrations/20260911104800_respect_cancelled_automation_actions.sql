...
create or replace function public.run_insight_automation(p_company uuid) returns integer
language plpgsql security invoker set search_path='' as $$
declare auto public.automations; ins public.company_insights; run_id uuid; action_id uuid; outcome jsonb; action_state text; executed integer := 0;
begin
 ...
 for ins in select * from public.company_insights where company_id=p_company and user_id=auth.uid() and status='new' order by created_at limit 20 loop
  ...
  select status into action_state from public.ai_actions where id=action_id;
  if action_state not in ('awaiting_approval','approved','completed') then
   update public.automation_runs set status='skipped',result=jsonb_build_object('reason','Existing action was rejected or failed. Founder review required.'),completed_at=now() where id=run_id;
   continue;
  end if;
