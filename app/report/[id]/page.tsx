...
type ReportContent = {
  ...
  sources?: Source[]; evidence?: Evidence; generatedBy?: "gemini" | "fallback";
...
export default async function Report({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="app-shell report-shell">
    ...
    <section className="report">
      ...
      <div className="verdict-row"><div className="score"><small>BUSINESS SCORE</small><strong>{report.score}</strong><span>/100</span></div><div className="verdict"><small>RECOMMENDATION</small><h2><i></i>{report.verdict}</h2><p>{verdictLine}</p></div><div className="confidence"><small>RESEARCH STATUS</small><b>{researchStatus}</b><p>Claims are labelled to separate evidence from estimates.</p></div></div>
      ...
      <section className="agent-strip"><div><span>THE ROOM</span><h2>Six specialists reviewed this idea.</h2></div><div className="agent-chips">{agents.map(([name, role, code], index) => <article className={`agent-chip agent-${index}`} key={name}><b>{code}</b><div><strong>{name}</strong><small>{role}</small></div></article>)}</div></section>
      <div className="report-grid">
        ...
        <aside>
          <div className="side-card"><span>YOUR NEXT MOVE</span><h3>Talk to 10 customers this week.</h3><p>Ask about their current workaround, what it costs them, and what would make them switch.</p><ReportActions plan={plan} mode="plan" /></div>
          <div className="source-card"><span>RESEARCH TRACE</span><p><i className="verified"></i> Verified information</p><p><i className="estimate"></i> AI estimates</p><p><i className="assume"></i> Assumptions</p><hr />{sources.map((source, index) => source.url ? <a key={source.url + index} href={source.url} target="_blank" rel="noreferrer" className="source-link"><small>↗ {source.title}</small><em>{source.domain}</em></a> : <small key={source.title + index} className="source-plain">{source.title}</small>)}</div>
        </aside>
      </div>
