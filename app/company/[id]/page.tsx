...
export default async function CompanyOverview({params}:{params:Promise<{id:string}>}) {
 ...
 if(!ctx) notFound();
 const [data,brief,missions,tasks]=await Promise.all([loadIntelligence(db,id),getOrCreateDailyBrief(db,id,userId),db.from('missions').select('id,objective,progress').eq('company_id',id).eq('status','active'),db.from('tasks').select('id,title,status,priority').eq('company_id',id)]);
 ...
 return <V3Dashboard companyId={id} name={ctx.company.name} displayName={name.split(' ')[0]} data={{...data,unavailable:data.unavailable||!!missions.error||!!tasks.error}} brief={brief} missions={missions.data??[]} tasks={tasks.data??[]} goal={ctx.primaryGoal}/>;
}
