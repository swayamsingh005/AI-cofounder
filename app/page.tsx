...
export default function Home() {
  ...
  return <main>
    <nav><a className="brand" href="/"><span className="brand-mark">✦</span> AI Co-Founder</a><div className="nav-links"><a href="#team">Meet the team</a><a href="/pricing">Pricing</a><a href="/reports">My reports</a><ThemeToggle/><button className="signin" onClick={() => router.push("/auth")}>Sign in <span>↗</span></button></div></nav>
    <section className="hero">
      ...
      <div className="character-stage"><div className="star star-a">✦</div><div className="star star-b">✦</div><div className="star star-c">✦</div><div className="speech speech-top"><span className="live-dot"></span> I&rsquo;ll find the market signal.</div><FounderCharacter className="sage" face="⌁" label="Mira" role="Market"/><FounderCharacter className="core" face="●" label="Nova" role="Your AI co-founder"/><FounderCharacter className="spark" face="◒" label="Theo" role="Strategy"/><div className="speech speech-bottom">What if we tested this first? <b>↗</b></div><div className="floor"><span>THE CO-FOUNDER ROOM</span></div></div>
      <form onSubmit={submit} className={prompt ? "idea-box needs-idea" : "idea-box"}><div className="idea-icon">✦</div><div className="idea-content"><label htmlFor="idea">Tell the room what you&rsquo;re thinking of building</label><textarea id="idea" value={idea} onChange={(e) => { setIdea(e.target.value); setPrompt(""); }} placeholder="A rough idea is perfect. Try: ‘a tool that helps…’"/></div><button type="submit">Start the conversation <b>→</b></button>{prompt && <p className="idea-warning">{prompt}</p>}</form>
