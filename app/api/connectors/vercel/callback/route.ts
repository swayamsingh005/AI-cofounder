...
export async function GET(request:Request) {
  ...
  try {
    const response=await fetch('https://api.vercel.com/v2/oauth/access_token',{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify({client_id:clientId,client_secret:secret,code,redirect_uri:new URL('/api/connectors/vercel/callback',request.url).toString()}),cache:'no-store',signal:AbortSignal.timeout(15000)});const token=await response.json() as {access_token?:string;team_id?:string;user_id?:string};if(!response.ok||!token.access_token)throw new Error('exchange');
