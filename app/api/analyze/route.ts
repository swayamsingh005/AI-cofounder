...
import { fallbackCeoPlan, normalizeCeoPlan, type CeoPlan } from "../../../lib/agents/ceo";
...
Required JSON shape (all fields required):
{
  ...
  "ceoPlan": {"businessType":string,"primaryObjective":string,"needsSoftware":boolean,"selectedAgents":[{"agentId":"research"|"coding"|"marketing","reason":string,"sequence":integer}],"excludedAgents":[{"agentId":"research"|"coding"|"marketing","reason":string}],"rationale":string},
...
Act as the CEO Agent when producing "ceoPlan". Choose specialists from the actual business need. Never assign Coding merely because every business can have a website. Set needsSoftware true and select Coding only when software is part of the product or the stated objective explicitly requires technical work. Research should precede dependent work when important facts remain unknown. Marketing is for positioning, acquisition or content. Explain both selections and exclusions.`;
